...
class Enemy(Plane):
    ...
    def __init__(self, kind, max_speed, *groups):
        ...
        if kind == 0:
            ...
            super().__init__(10, 1, 6000, "enemy2_down.wav",
                             ["enemy2.png"],
                             "enemy2_hit.png",
                             ["enemy2_down%d.png" % i for i in range(1, 5)],
                             *groups)
        else:
            super().__init__(30, 1, 15000, "enemy3_down.wav",
                             ["enemy3_n1.png", "enemy3_n2.png"],
                             "enemy3_hit.png",
                             ["enemy3_down%d.png" % i for i in range(1, 5)],
                             *groups)
        # 3. 调用飞机方法，设置敌机初始位置和速度
        self.reset_plane()
...
class Hero(Plane):
    ...
    def update(self, *args):
        """更新英雄的图像及矩形区域
        :param args: 0 更新图像标记 1 水平移动计数 2 垂直移动基数
        """
        # 调用父类方法更新飞机图像 - 注意 args 需要拆包
        super().update(*args)
